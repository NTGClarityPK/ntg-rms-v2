...
async function seedFoodItems(tenantId: string, categoryIds: string[]): Promise<{ foodItemIds: string[]; foodItemMap: Map<string, string> }> {
  ...
  for (const item of foodItems) {
    ...
    if (error) {
      console.error(`   ❌ Failed to create food item "${item.name}":`, error.message);
      continue;
    }

      itemId = newItem.id;
    console.log(`   ✅ Created food item: ${item.name} (${item.base_price} IQD)`);
    }

    foodItemIds.push(itemId);
    foodItemMap.set(item.name, itemId);
    ...
    if (item.menu_type) {
      ...
      if (!existingMenuAssignment) {
        ...
        if (menuError) {
          console.error(`   ⚠️  Failed to add "${item.name}" to menu "${item.menu_type}":`, menuError.message);
        } else {
          console.log(`   ✅ Added "${item.name}" to menu "${item.menu_type}"`);
        }
      }
    }
  }
...
async function seedIngredients(tenantId: string): Promise<{ ingredientIds: string[]; ingredientMap: Map<string, string> }> {
  ...
  const ingredients = [
    ...
    { name: 'Lamb', category: 'meats', unit_of_measurement: 'kg', current_stock: 30, minimum_threshold: 5, cost_per_unit: 15000 },
    { name: 'Beef', category: 'meats', unit_of_measurement: 'kg', current_stock: 25, minimum_threshold: 5, cost_per_unit: 12000 },
    { name: 'Fish', category: 'meats', unit_of_measurement: 'kg', current_stock: 20, minimum_threshold: 5, cost_per_unit: 10000 },
    { name: 'Rice', category: 'grains', unit_of_measurement: 'kg', current_stock: 100, minimum_threshold: 20, cost_per_unit: 2000 },
    { name: 'Chickpeas', category: 'legumes', unit_of_measurement: 'kg', current_stock: 40, minimum_threshold: 10, cost_per_unit: 3000 },
    { name: 'Eggplant', category: 'vegetables', unit_of_measurement: 'kg', current_stock: 15, minimum_threshold: 5, cost_per_unit: 1500 },
    { name: 'Tomatoes', category: 'vegetables', unit_of_measurement: 'kg', current_stock: 30, minimum_threshold: 10, cost_per_unit: 2000 },
    { name: 'Onions', category: 'vegetables', unit_of_measurement: 'kg', current_stock: 25, minimum_threshold: 5, cost_per_unit: 1000 },
    { name: 'Garlic', category: 'vegetables', unit_of_measurement: 'kg', current_stock: 10, minimum_threshold: 2, cost_per_unit: 5000 },
    { name: 'Tahini', category: 'condiments', unit_of_measurement: 'kg', current_stock: 20, minimum_threshold: 5, cost_per_unit: 6000 },
    { name: 'Olive Oil', category: 'condiments', unit_of_measurement: 'liter', current_stock: 50, minimum_threshold: 10, cost_per_unit: 8000 },
    { name: 'Lemon', category: 'vegetables', unit_of_measurement: 'kg', current_stock: 20, minimum_threshold: 5, cost_per_unit: 1500 },
    { name: 'Parsley', category: 'vegetables', unit_of_measurement: 'bunch', current_stock: 50, minimum_threshold: 10, cost_per_unit: 500 },
    { name: 'Mint', category: 'vegetables', unit_of_measurement: 'bunch', current_stock: 40, minimum_threshold: 10, cost_per_unit: 500 },
    { name: 'Bulgur', category: 'grains', unit_of_measurement: 'kg', current_stock: 30, minimum_threshold: 5, cost_per_unit: 2500 },
    { name: 'Flour', category: 'grains', unit_of_measurement: 'kg', current_stock: 80, minimum_threshold: 20, cost_per_unit: 1500 },
    { name: 'Yogurt', category: 'dairy', unit_of_measurement: 'kg', current_stock: 40, minimum_threshold: 10, cost_per_unit: 3000 },
    { name: 'Cheese', category: 'dairy', unit_of_measurement: 'kg', current_stock: 20, minimum_threshold: 5, cost_per_unit: 7000 },
    { name: 'Spices Mix', category: 'spices', unit_of_measurement: 'kg', current_stock: 15, minimum_threshold: 3, cost_per_unit: 10000 },
  ];
  ...
  for (const ing of ingredients) {
    ...
    if (existing) {
      console.log(`   ⏭️  Ingredient "${ing.name}" already exists`);
      ...
      if (error) {
        console.error(`   ❌ Failed to create ingredient "${ing.name}":`, error.message);
        continue;
      }

      ingredientId = newIng.id;
      console.log(`   ✅ Created ingredient: ${ing.name} (${ing.current_stock} ${ing.unit_of_measurement})`);
    }
...
async function seedAddOns(tenantId: string): Promise<{ addOnGroupIds: string[]; addOnIds: string[] }> {
  ...
  for (const group of addOnGroups) {
    ...
    if (groupError) {
      console.error(`   ❌ Failed to create add-on group "${group.name}":`, groupError.message);
      continue;
    }
    ...
    for (const addOn of addOns) {
      ...
      if (addOnError) {
        console.error(`   ❌ Failed to create add-on "${addOn.name}":`, addOnError.message);
        continue;
      }
...
async function seedCustomerAddresses(customerIds: string[]): Promise<void> {
  ...
  const addresses = [
    { customerIndex: 0, label: 'Home', address: 'Al-Karada, Street 14, Building 25', city: 'Baghdad', is_default: true },
    { customerIndex: 1, label: 'Work', address: 'Al-Mansour, Business District, Office 301', city: 'Baghdad', is_default: false },
    { customerIndex: 2, label: 'Home', address: 'Al-Jadriya, University Area, Apartment 5', city: 'Baghdad', is_default: true },
    { customerIndex: 3, label: 'Home', address: 'Al-Zawraa, Residential Complex, Block 3', city: 'Baghdad', is_default: true },
  ];
